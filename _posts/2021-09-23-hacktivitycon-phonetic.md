---
layout: post
title: "H@cktivitycon Writeup: Phonetic"
image: ''
...
This post, specifically, is dedicated to the malware analysis challenge, Phonetic. This one really upset me because I finally realized what I was doing wrong only minutes before the CTF was over, and I got the flag one minute too late. However, I was pretty happy with my ability to go through this malware sample alone. I'll begin by cleaning up the original php file and identify what exactly the code is doing. I'll then recreate the sample in Python, but I'll tell it to print the payload out rather than execute it. I'll finally find some more encoded text in the payload, but find an online decoder to grab the flag.

### Description
`It's easy to find the flag... just sound it out!`

## Deobfuscating Initial File
My first exposure to doing some real malware analysis was [this video](https://www.youtube.com/watch?v=3Q9-X_NRlJc) by John Hammond, where he goes into deobfuscating and understanding some VBS script, and I highly recommend this for anyone who wants to begin understanding the process of malware analysis. I myself am no expert in this area, but at a very basic level, we're just reading code and trying to analyze the function of it.
As much as I would like to explain my step by step process, that's really hard to pull off in a text format, so I'm going to introduce the initial file, break down certain aspects, and work from there. 

### High-Level Review
Without further ado, here is the original file, minus the super long base64 strings.
```php
<?php  
...
$gbaylYLd6204 = "[VERY LONG BASE64 STRING]";
$fsPwhnfn8423 = "";
$oZjuNUpA325 = "";
...
If you've never seen actual malware code before, this is probably really jarring. However, the easiest way of going about this is looking at a line or two, renaming the variables to be something more clear, and move down. If you have some epihany about a certain function, you go back and you rename and clarify what you're looking at. I'll go ahead and fix the formatting of the top of the file.
...
### Function: deGRi
From looking at the `deGRi` function above, we see that it's doing some kind of nested for loop work, and returning some string that it's concatenating to (the `.=` sign). After going through, I deobfuscated it to look like this.
```php
function xor_strings($param1, $param2 = '') { 
	$copy1 = $param1; 
	$concated = ''; 
	for ($index1 = 0; $index1 < strlen($copy1);) { 
		for ($index2 = 0; $index2 < strlen($param2) && $index1 < strlen($copy1); $index2++, $index1++) { 
			$concated .= $copy1[$index1] ^ $param2[$index2]; 
		} 
	} 
	return $concated; 
}
...
I started with the parameters, and noticed that one of them was getting copied. After that, we enter this loop where we do an XOR between every character in each input, and we add that to a final string. I called the function `xor_strings` and I moved on to the main chunk.

### Main Body
This is what the other half of the program looked like.
```php
$lBuAnNeu5282 = ")o4la2cih1kp97rmt*x5dw38b(sfy6;envguz_jq/.0";
$gbaylYLd6204 = "[VERY LONG BASE64 STRING]";
$fsPwhnfn8423 = "";
...
Although this looks intimidating at first, it's actually not very complex. The first two lines here just define strings (running a base64 decoder on the second one doesn't return much). The next two are empty strings that appear to also serve the purpose of being concatenated, according to what looks to be indicies of the strings. After this, we use the translated character codes to call functions (apparently you can do this in php and I did not know that until this challenge), where we finally call some function at the end.
The comments don't have much info in them either; I could try to decode them for a hint, but I think that's just a waste of time. After working through this segment, we get the following code.

```php
$mystery1 = ")o4la2cih1kp97rmt*x5dw38b(sfy6;envguz_jq/.0";
$big_mystery2 = "[VERY LONG BASE64 STRING]";
$concat1 = "";
$concat2 = "";
foreach([24,4,26,31,29,2,37,20,31,6,1,20,31] as $k){
   $concat1 .= $mystery1[$k];
...
$k = $concat2('n'.''.''.'o'.''.''.'i'.''.'t'.''.'c'.''.'n'.''.'u'.'f'.''.''.''.''.'_'.''.''.''.'e'.''.'t'.''.'a'.''.'e'.''.''.''.''.'r'.''.''.''.''.'c');
$c = $k("/*XAjqgQvv4067*/", $concat1( deGRi($concat1($big_mystery2), "tVEwfwrN302")));
$c();

/*[irrelevant]*/
```

## Recreating it in Python
We can only do static analysis for so long. I've deobfuscated the file to the point where I can understand the general function of the code, but I have no clue what is in the big strings. Although this is a CTF and they probably wouldn't give us live samples, I'm certainly not going to try and run this on my machine. What we can do, however, is try and extract whatever payload is being executed in `$c`.

I originally recreated this in Python, but I think if I'd do it again I'd have gone with a language like Java, for the sole reason that Python's loop syntax is starkly different than other languages (and that issue plagued me for at least 4 hours when doing this in the event). Below is stage one of my recreation, so I knew what mystery functions I was going after.

```python
#!/usr/bin/python3

def xor_strings(param1, param2=''):
	copy1 = param1
	concated = ''
	i, j = 0, 0
	while i < len(param1):
		while j < len(param2):
			concated += copy1[i] ^ param2[j]
	return concated
	
if __name__ == "__main__":
	mystery1 = ")o4la2cih1kp97rmt*x5dw38b(sfy6;envguz_jq/.0"
	big_mystery2 = "[MUCHO LONG]"
	concat1 = ""
	concat2 = ""
...
```bash
remnux@remnux:~/ctf/hacktivitycon-2021/malware/phonetic$ python3 challenge.py 
CONCAT1: base64_decode
CONCAT2: strrev
```

Looks like it's just a fancy way of encoding the functions. With this new info, we can see that there's an additional list that calls what we found to be the `strrev` function. If we reverse that, we get the output `create_function`. Now, if we put all of the puzzle pieces together, we find that the whole code is building up to this line.

```php
$payload = create_function("/*XAjqgQvv4067*/", base64_decode( xor_strings(base64_decode($big_mystery2), "tVEwfwrN302")));
$payload();
```

I'm going to implement this in Python, except without the `create_function`, because I'm not about to start a beacon on this VM (and just to be thorough). After some debugging of the `xor_strings()` function, we end up with this.
...
def xor_strings(param1, param2=''):
	...
	while i < len(copy1):
		j = 0
		while j < len(param2) and i < len(copy1):
			# debug statements
			#print(f"[+] STATUS: {i+1}/{len(param1)} chars, ", end="")
			#print(f"{j+1}/{len(param2)} chars")
			concated += chr(copy1[i] ^ ord(param2[j]))
			i += 1
			j += 1
	return concated
	
if __name__ == "__main__":
	print("[+] Starting payload decryption...")
	mystery1 = ")o4la2cih1kp97rmt*x5dw38b(sfy6;envguz_jq/.0"
	mystery2 = "[BIG BIG BASE64 STRING YOU GET IT]"
	concat1 = "" # base64 decode
	concat2 = "" # strrev
	...
	for k in [26,16,14,14,31,33]:
		concat2 += mystery1[k]	
	"""
	stuff = xor_strings(base64.b64decode(big_mystery2), "tVEwfwrN302")
	#print(stuff)
	payload = base64.b64decode(stuff)
